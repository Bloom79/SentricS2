...
class RecurringObligationService:
    ...
    ITALIAN_OBLIGATIONS = [
        ...
        },
        {
            ...
            "conditional": lambda plant: plant.power_kw and plant.power_kw > 100,  # Simplified check
        },
        {
    ...
    def create_recurring_obligations_for_plant(
        ...
        for obligation_config in RecurringObligationService.ITALIAN_OBLIGATIONS:
            ...
            existing = db.query(RecurringObligation).filter(
            ...
            if existing:
                logger.info(f"Obligation {obligation_config['obligation_type']} already exists for plant {plant_id}")
                continue
            ...
            logger.info(f"Created obligation {obligation_config['obligation_type']} for plant {plant_id}")
        ...
        if recurrence_pattern == "annual":
            ...
            if "month" in base_deadline and "day" in base_deadline:
                ...
                if deadline_this_year < now:
                    deadline_this_year = datetime(year + 1, base_deadline["month"], base_deadline["day"])
    ...
    def mark_completed(
        ...
        obligation = db.query(RecurringObligation).filter(
        ...
        logger.info(f"Marked obligation {obligation_id} as completed. Next due: {obligation.next_due_date}")
        
        return obligation
    
    @staticmethod
    def auto_create_workflows(
        db: Session,
        tenant_id: str,
        days_ahead: int = 30
    ) -> List[Workflow]:
