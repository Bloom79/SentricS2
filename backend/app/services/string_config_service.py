...
class StringConfigService:
    ...
    def assign_panels_to_string(
        ...
        try:
            ...
            for panel_id_str, assigned_string in list(string_assignments.items()):
                if assigned_string == string_number and str(panel_id_str) not in [str(p) for p in panel_ids]:
            ...
            logger.info(f"Assigned {len(panel_ids)} panels to string {string_number} in array {array_id}")
            return array
    ...
    def get_string_details(
        ...
        try:
            ...
            assigned_panel_ids = [
                int(panel_id) for panel_id, str_num in string_assignments.items()
            ...
            panels = db.query(Asset).filter(
                and_(
                    Asset.id.in_(assigned_panel_ids),
                    Asset.tenant_id == tenant_id
                )
            ).all()
            ...
            total_voltage = sum(
                (p.dynamic_attributes or {}).get("nominal_voltage", 0) or 0
                for p in panels
            )
            nominal_current = panels[0].dynamic_attributes.get("nominal_current", 0) if panels else 0
            ...
            return {
                ...
                "status": "full" if len(panels) == panels_per_string else ("partial" if len(panels) > 0 else "empty"),
        ...
        try:
            ...
            for string_num in range(1, number_of_strings + 1):
                ...
                strings.append({
                    ...
                    "max_panels": panels_per_string,
                    "status": "full" if len(assigned_panel_ids) == panels_per_string else (
                        "partial" if len(assigned_panel_ids) > 0 else "empty"
                    ),
                })
