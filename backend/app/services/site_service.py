...
class SiteService:
    ...
    @staticmethod
    def create_site(db: Session, site_data: Dict[str, Any], tenant_id: str) -> Site:
        """Create a new site"""
        try:
            site = Site(
                tenant_id=tenant_id,
    ...
    @staticmethod
    def get_site(
        db: Session, site_id: int, tenant_id: str, include_relations: bool = True
    ) -> Optional[Site]:
        """Get site by ID with optional relations"""
        try:
            query = db.query(Site).filter(and_(Site.id == site_id, Site.tenant_id == tenant_id))
    ...
    def list_sites(
        ...
        try:
            ...
            sites = (
                query.options(selectinload(Site.plants))  # Eager load plants for count
                .offset(skip)
                .limit(limit)
                .all()
            )
    ...
    @staticmethod
    def update_site(
        db: Session, site_id: int, site_data: Dict[str, Any], tenant_id: str
    ) -> Optional[Site]:
        """Update site"""
        try:
            site = (
                db.query(Site).filter(and_(Site.id == site_id, Site.tenant_id == tenant_id)).first()
            )
    ...
    @staticmethod
    def delete_site(db: Session, site_id: int, tenant_id: str, soft: bool = True) -> bool:
    ...
    @staticmethod
    def get_site_stats(db: Session, site_id: int, tenant_id: str) -> Dict[str, Any]:
        """Get site statistics"""
        try:
            site = (
                db.query(Site).filter(and_(Site.id == site_id, Site.tenant_id == tenant_id)).first()
            ...
            plants_count = (
                ...
                .filter(and_(Plant.site_id == site_id, Plant.tenant_id == tenant_id))
                .scalar()
                or 0
            )
            ...
            total_capacity = (
                db.query(func.sum(Plant.power_kw))
                .filter(and_(Plant.site_id == site_id, Plant.tenant_id == tenant_id))
                .scalar()
                or 0.0
            )
            ...
            total_storage_capacity = (
                sum(s.capacity_kwh for s in site.storage_units) if site.storage_units else 0.0
            )
    ...
    @staticmethod
    def get_site_energy_flow(db: Session, site_id: int, tenant_id: str) -> Optional[EnergyFlow]:
