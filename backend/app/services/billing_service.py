...
from app.schemas.billing import (
    ...
    BillingTransactionCreate, BillingTransactionUpdate,
...
class BillingService:
    ...
    def calculate_settlement(
        ...
        incentive_rate = BillingService._calculate_incentive_rate(
            total_capacity_kw, cer.region
        )
        ...
        total_grid_fees = (energy_calc["grid_import"] + energy_calc["grid_export"]) * BillingService.DEFAULT_GRID_FEE_RATE
        ...
        for member_id, allocation_data in energy_calc["member_allocation"].items():
            ...
            member_amount = member_incentives - member_grid_fees - member_community_fund - member_energy_cost
    ...
    def generate_billing_statements(
        ...
        for member in members:
            ...
            energy_data = settlement.calculation_data.get("energy_calculation", {}).get("member_allocation", {}).get(member_id_str, {})
        ...
        logger.info(f"Generated {len(statements)} billing statements for settlement {settlement_id}")
        return statements
    ...
    def create_billing_transaction(
        ...
        transaction = BillingTransaction(
            ...
            status=PaymentStatus.PENDING if transaction_data.transaction_type == TransactionType.PAYMENT else PaymentStatus.COMPLETED,
        ...
        if transaction_data.statement_id:
            statement = db.query(BillingStatement).filter(
                and_(
        ...
        logger.info(f"Created billing transaction {transaction.id} for member {transaction_data.member_id}")
        return transaction
    ...
    def get_member_balances(
        ...
        for member in members:
            ...
            last_transaction = db.query(BillingTransaction).filter(
                and_(
            ...
            ).order_by(BillingTransaction.payment_date.desc()).first()
