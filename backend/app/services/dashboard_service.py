...
class DashboardService:
    ...
    def get_dashboard_stats(
        ...
        base_filter = lambda model: and_(
            model.tenant_id == tenant_id,
            model.deleted_at.is_(None)
            )
        
        # Optimized: Single query for plant stats using CASE statements
        plant_stats = db.query(
            func.count(Plant.id).label('total'),
            func.sum(case((Plant.status == PlantStatusEnum.IN_OPERATION, 1), else_=0)).label('active'),
            func.sum(Plant.power_kw).label('total_capacity')
        ).filter(base_filter(Plant)).first()
        ...
        ).filter(base_filter(Asset)).first()
        ...
        workflow_stats = db.query(
            ...
            func.sum(case((Workflow.status == WorkflowStatusEnum.IN_PROGRESS, 1), else_=0)).label('active')
        ).filter(base_filter(Workflow)).first()
        
        total_workflows = workflow_stats.total or 0
        active_workflows = workflow_stats.active or 0
        
        # Fixed: Compliance statistics query
        overdue_compliance = db.query(func.count(ComplianceRecord.id)).filter(
                and_(
        ...
        ).filter(base_filter(Document)).first()
    ...
    def get_recent_activity(
        ...
        # Recent plants
        recent_plants = db.query(Plant).filter(
            and_(
                Plant.tenant_id == tenant_id,
                Plant.deleted_at.is_(None)
            )
        ).order_by(Plant.created_at.desc()).limit(limit).all()
