...
class WorkflowTemplateService:
    ...
    def create_workflow_from_template(
        ...
        workflow = Workflow(
            ...
            type=WorkflowTypeEnum[template.workflow_type] if template.workflow_type else WorkflowTypeEnum.COMPLIANCE,
        ...
        template_phases = db.query(WorkflowTemplatePhase).filter(
            and_(
                WorkflowTemplatePhase.template_id == template_id,
                WorkflowTemplatePhase.tenant_id == tenant_id,
                WorkflowTemplatePhase.deleted_at.is_(None)
            )
        ).order_by(WorkflowTemplatePhase.order).all()
