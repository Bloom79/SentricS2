...
class EnergyTransaction(BaseModel):
    ...
    member_id = Column(Integer, ForeignKey("cer_members.id"), nullable=True)  # Optional - can be CER-level
    plant_id = Column(Integer, ForeignKey("plants.id"), nullable=True)  # Optional - if from specific plant
...
class EnergySharingCalculation(BaseModel):
    ...
    def __repr__(self):
        return f"<EnergySharingCalculation CER {self.cer_id} {self.period_start}->{self.period_end}>"
