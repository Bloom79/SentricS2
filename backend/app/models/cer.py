...
from sqlalchemy import Column, Integer, String, Float, Boolean, DateTime, JSON, ForeignKey, Enum, Text
...
class CER(BaseModel):
    ...
    # Relationships
    members = relationship("CERMember", back_populates="cer", cascade="all, delete-orphan", lazy="select")
    plants = relationship("Plant", back_populates="cer", lazy="select")
    participation_requests = relationship("CERParticipationRequest", back_populates="cer", cascade="all, delete-orphan", lazy="select")
    energy_transactions = relationship("EnergyTransaction", back_populates="cer", cascade="all, delete-orphan", lazy="select")
    energy_sharing_calculations = relationship("EnergySharingCalculation", back_populates="cer", cascade="all, delete-orphan", lazy="select")
    compliance_requirements = relationship("ComplianceRequirement", back_populates="cer", lazy="select")
    ...
    billing_statements = relationship("BillingStatement", back_populates="cer", cascade="all, delete-orphan", lazy="select")
    invoices = relationship("Invoice", back_populates="cer", cascade="all, delete-orphan", lazy="select")
    billing_transactions = relationship("BillingTransaction", back_populates="cer", cascade="all, delete-orphan", lazy="select")
    settlements = relationship("Settlement", back_populates="cer", cascade="all, delete-orphan", lazy="select")
...
class CERMember(BaseModel):
    ...
    # Energy Profile
    load_profile_type = Column(String(50), nullable=False)  # residential, commercial, industrial, custom
    ...
    assets = relationship("CERMemberAsset", back_populates="member", cascade="all, delete-orphan", lazy="select")
    energy_transactions = relationship("EnergyTransaction", back_populates="member", cascade="all, delete-orphan", lazy="select")
    billing_statements = relationship("BillingStatement", back_populates="member", cascade="all, delete-orphan", lazy="select")
    invoices = relationship("Invoice", back_populates="member", cascade="all, delete-orphan", lazy="select")
    billing_transactions = relationship("BillingTransaction", back_populates="member", cascade="all, delete-orphan", lazy="select")
...
class CERParticipationRequest(BaseModel):
    ...
    # Request info
    status = Column(Enum(ParticipationRequestStatus), nullable=False, default=ParticipationRequestStatus.PENDING)
