...
class Plant(BaseModel):
    ...
    # Relationships
    tenant = relationship("Tenant", back_populates="plants", foreign_keys="[Plant.tenant_id]", primaryjoin="Plant.tenant_id == Tenant.id", lazy='select')
    site = relationship("Site", back_populates="plants")  # NEW: Site relationship
    registry = relationship("PlantRegistry", back_populates="plant", uselist=False)  # One-to-one via PlantRegistry.plant_id (unique)
    ...
    recurring_obligations = relationship("RecurringObligation", back_populates="plant", foreign_keys="[RecurringObligation.plant_id]")
    
    # NEW: Visual Designer layout
    layout = relationship("PlantLayout", back_populates="plant", uselist=False, cascade="all, delete-orphan")
...
class PlantRegistry(BaseModel):
    ...
    plant = relationship("Plant", back_populates="registry", foreign_keys="[PlantRegistry.plant_id]", uselist=False)
