...
# Member endpoints
@router.post("/communities/{cer_id}/members", response_model=CERMemberResponse, status_code=status.HTTP_201_CREATED)
...
@router.post("/participation-requests", response_model=CERParticipationRequestResponse, status_code=status.HTTP_201_CREATED)
...
@router.get("/communities/{cer_id}/participation-requests", response_model=List[CERParticipationRequestResponse])
...
async def get_cer_compliance(
    ...
    try:
        ...
        linked_plants = db.query(Plant).filter(
            and_(
                Plant.cer_id == cer_id,
        ...
        for plant in linked_plants:
            try:
                ...
                plant_compliance_summary.append({
                    ...
                    "compliance_status": "compliant" if len(plant_ovr) == 0 else "non_compliant"
                })
            except Exception as e:
        ...
        def serialize_record(rec):
            try:
                return {
                    ...
                    "completed_date": rec.completed_date.isoformat() if rec.completed_date else None,
                ...
                return {"id": rec.id, "requirement_id": rec.requirement_id, "status": str(rec.status)}
...
async def get_cer_documents_overview(
    ...
    try:
        ...
        return {
            ...
            "expiring_soon_count": len([d for d in documents if hasattr(d, 'days_until_expiry') and d.days_until_expiry is not None and 0 < d.days_until_expiry <= 30])
        }
    except HTTPException:
        raise
    except Exception as e:
...
@router.post("/communities/{cer_id}/members/{member_id}/assets", response_model=CERMemberAssetResponse, status_code=status.HTTP_201_CREATED)
async def create_member_asset(
    ...
    try:
        ...
        asset_create = CERMemberAssetCreate(
            **asset_data.dict(),
            member_id=member_id,
            cer_id=cer_id
        )
...
@router.get("/communities/{cer_id}/members/{member_id}/assets", response_model=List[CERMemberAssetResponse])
async def list_member_assets(
    ...
    try:
        ...
        logger.error(f"Error listing assets for member {member_id} in CER {cer_id}: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Internal server error")


@router.get("/communities/{cer_id}/members/{member_id}/assets/{asset_id}", response_model=CERMemberAssetResponse)
...
@router.put("/communities/{cer_id}/members/{member_id}/assets/{asset_id}", response_model=CERMemberAssetResponse)
...
@router.delete("/communities/{cer_id}/members/{member_id}/assets/{asset_id}", status_code=status.HTTP_204_NO_CONTENT)
